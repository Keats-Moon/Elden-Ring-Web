...
突发奇想做的工具，可能没什么用
它可以：
读取**你自己** Steam 账号上的《艾尔登法环》（AppID `1245620`）游戏数据：成就进度、追忆 Boss 击杀记录、游玩时长，并可选择性解析本地存档，获得完整的武器 / 防具 / 护符 / 道具收集清单。
...
> 注意：**非追忆 Boss 不在 Steam 成就系统内**。像大树守卫、蒙格温王朝的蒙格这类 Boss 没有任何成就，因此「Boss 与结局」页里不会出现它们。"不在列表里"不等于"没打过"，界面上也明确写了这一点。
...
**关键的诚实说明**：OpenID 只解决"你是谁"，**不绕过隐私设置**。如果用户的「游戏详情」是私密，Steam 依然拒绝返回成就——这不是本站的限制，是 Steam 的规则。界面上会给出明确的操作指引。
...
> 这一点极具迷惑性 —— 日志说"重载了"，实际没生效。改完请 `Ctrl+C` 后重新 `npm run dev`。
